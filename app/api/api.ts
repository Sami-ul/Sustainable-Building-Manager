...
export const fetchBuildingAverageData = async (buildingId: number): Promise<BuildingAverageData> => {
  const response = await fetch(`http://127.0.0.1:3001/buildings/${buildingId}/average`);
  if (!response.ok) {
...
export const fetchEnergyTrendsData = async (buildingId: number): Promise<{ Date: string; "Energy Consumption (kWh)": number }[]> => {
  const response = await fetch(`http://127.0.0.1:3001/graphs/energy_trends/${buildingId}`);
  if (!response.ok) {
...
export const fetchOccupancyTrendsData = async (buildingId: number): Promise<{ Date: string; "Occupancy (People)": number }[]> => {
  const response = await fetch(`http://127.0.0.1:3001/graphs/occupancy_trends/${buildingId}`);
  if (!response.ok) {
...
export const fetchCarbonEmissionsTrendsData = async (buildingId: number): Promise<{ Date: string; "Carbon Emissions (kg CO2)": number }[]> => {
  const response = await fetch(`http://127.0.0.1:3001/graphs/carbon_emissions_trends/${buildingId}`);
  if (!response.ok) {
...
export const fetchHvacUsageTrendsData = async (buildingId: number): Promise<{ Date: string; "HVAC Usage (Hours)": number }[]> => {
  const response = await fetch(`http://127.0.0.1:3001/graphs/hvac_usage_trends/${buildingId}`);
  if (!response.ok) {
...
export const fetchWaterUsageTrendsData = async (buildingId: number): Promise<{ Date: string; "Water Usage (Gallons)": number }[]> => {
  const response = await fetch(`http://127.0.0.1:3001/graphs/water_usage_trends/${buildingId}`);
  if (!response.ok) {
...
export const fetchEnergyVsOccupancyData = async (
    ...
    count: number;  }[]> => {
    const response = await fetch(`http://127.0.0.1:3001/graphs/energy_vs_occupancy/${buildingId}`);
    if (!response.ok) {
